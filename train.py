...
from model import CombinedSpatiotemporalModel
...
def validate(model, test_loader, t0, t1, device):
    ...
    msape_meter = AverageMeter()
    loglik_meter = AverageMeter()

    with torch.no_grad():
        for batch in test_loader:
            event_times, spatial_events, input_mask, label = map(lambda x: cast(x, device), batch)
            num_events = input_mask.sum()
            pre_label, loglik  = model(event_times, spatial_events, input_mask, t0, t1)
            loglik = loglik.sum() / num_events

            msle_loss = get_msle(pre_label, label)
            msape = get_msape(pre_label, label)
            msle_meter.update(msle_loss.mean().item(), input_mask.shape[0])
            msape_meter.update(msape.mean().item(), input_mask.shape[0])
            loglik_meter.update(loglik.item(), num_events)

    model.train()

    return msle_meter.avg, msape_meter.avg, loglik_meter.avg
...
def _main(rank, world_size, args, savepath, logger):
    ...
    if rank == 0:
        if device.type == 'cuda':           #打印GPU信息
            logger.info('Found {} CUDA devices.'.format(torch.cuda.device_count()))
            for i in range(torch.cuda.device_count()):              #遍历GPU
                props = torch.cuda.get_device_properties(i)                 #获得每个GPU的信息
                logger.info('{} \t Memory: {:.2f}GB'.format(props.name, props.total_memory / (1024**3)))
        else:
            logger.info('WARNING: Using device {}'.format(device))

    t0, t1 = map(lambda x: cast(x, device), get_t0_t1(args.t1))                   #当前数据集的时间间隔的起点和终点
    print('t0:', t0, 't1:', t1)

    x_dim = args.cg_emb_dim  # 空间维度的dim
    ...
    model = DDP(model, device_ids=[rank], find_unused_parameters=True)              #转分布式
    ...
    for epoch in range(begin_epoch, args.max_epoch):
        ...
        for batch in batch_iter:
            ...
            msape = get_msape(pre_label, label)
            msape_meter.update(msape.mean().item(), N)

            loglik = loglik.sum() / num_events
            loglik_meter.update(loglik.item())

            # if args.loss:
            loss = msle_loss.mean() - 0.1*loglik.mean() + msape.mean()
            loss.backward()
            ...
            if rank == 0:
                ...
                tb_writer.add_scalar("train/msape", msape.mean().item(), itr)
            ...
            if itr % args.logfreq == 0:
                ...
                if rank == 0:
                    logger.info(
                        ...
                        f" | msape {msape_meter.val:.4f}({msape_meter.avg:.4f})"
        ...
        if rank == 0:
            pre_label_val, msape_val, val_loglik = validate(model, val_loader, t0, t1, device)
            pre_label_test, msape_test, test_loglik = validate(model, test_loader, t0, t1, device)
            ...
            if pre_label_val < best_val_msle:
                ...
                best_val_msape = msape_val
                best_test_msape = msape_test
                ...
                # ema.swap()
                logger.info(
                    f"[Test] Iter {itr} | Epoch {epoch}| Val msle {pre_label_val:.4f} | Val msape {msape_val:.4f} | Val loglik {val_loglik:.4f}"
                    f" | Test msle {pre_label_test:.4f} | Test msape {msape_test:.4f}| Test loglik {test_loglik:.4f}"
                    f" | Best Test msle {best_test_msle:.4f} | Best Val msle {best_val_msle:.4f}"
                    f" | Best Test msape {best_val_msape:.4f} | Best Val msape {best_test_msape:.4f}")

                tb_writer.add_scalar("val/msle", pre_label_val, itr)
                tb_writer.add_scalar("test/msle", pre_label_test, itr)
                tb_writer.add_scalar("val/msape", msape_val, itr)
                tb_writer.add_scalar("test/msape", msape_test, itr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cg_emb_dim", type=int, default=40)
    ...
    parser.add_argument("--model", type=str, choices=["ode_tpp_trans"],
                        default="ode_tpp_trans")
    parser.add_argument("--tpp_hdims", type=str, default="32")
    ...
    savepath = os.path.join(args.experiment_dir, args.data + '_t' + args.observation_time + '_s' + args.max_seq + '_t'
                            + str(args.t1) + '_d' + str(args.cg_emb_dim), experiment_name)
