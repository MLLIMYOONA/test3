...
from graphwave.sparse_matrix_factorization import *
...
def write_cascade(graphs, labels, filename, weight=True):
    ...
    with open(filename, 'wb') as f:
        pickle.dump((cascade_input, y_data), f)
...
def main(argv):
    ...
    graph_train = sequence2list(FLAGS.data + f'train_{FLAGS.observation_time}.txt')
    graph_val = sequence2list(FLAGS.data + f'val_{FLAGS.observation_time}.txt')
    graph_test = sequence2list(FLAGS.data + f'test_{FLAGS.observation_time}.txt')
    ...
    write_cascade(graph_train, label_train, FLAGS.data + f'train_t{FLAGS.observation_time}_s{FLAGS.max_seq}_d{FLAGS.cg_emb_dim}.pkl')
    print('Start writing val set into file.')
    write_cascade(graph_val, label_val, FLAGS.data + f'val_t{FLAGS.observation_time}_s{FLAGS.max_seq}_d{FLAGS.cg_emb_dim}.pkl')
    print('Start writing test set into file.')
    write_cascade(graph_test, label_test, FLAGS.data + f'test_t{FLAGS.observation_time}_s{FLAGS.max_seq}_d{FLAGS.cg_emb_dim}.pkl')
