...
class SpatioTemporalDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, train_set, test_set, train_label, test_label, x_dim, train):
    ...
    def __getitem__(self, index):
        return (self.dataset[index], self.dataset_label[index])


class CascadeData(SpatioTemporalDataset):

    def __init__(self, data='', split="train", observation_time="1800", seq_len=100, x_dim=40):
        assert split in ["train", "val", "test"]
        self.split = split

        with open(f'data/{data}/train_t{observation_time}_s{seq_len}_d{x_dim}.pkl', 'rb') as ftrain:
            train_cascade, train_label = pickle.load(ftrain)
        with open(f'data/{data}/val_t{observation_time}_s{seq_len}_d{x_dim}.pkl', 'rb') as fval:
            val_cascade, val_label = pickle.load(fval)
        with open(f'data/{data}/test_t{observation_time}_s{seq_len}_d{x_dim}.pkl', 'rb') as ftest:
            test_cascade, test_label = pickle.load(ftest)
        ...
        super().__init__(train_set, split_set, train_label, split_label, x_dim,  split == "train")
...
def spatiotemporal_events_collate_fn(data):
    ...
    return event_times, spatial_locations, mask, label
